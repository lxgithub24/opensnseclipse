...
<div class="row ">
    ...
    <link href="/opensnseclipse/Application/Ucenter/Static/css/invite.css" type="text/css" rel="stylesheet">
    <div class="row">
        <div class="col-xs-12">
            <p>&nbsp;</p>
            <h4><?php echo L('_INV_LIST_');?></h4>
            <hr class="oneuser-title-hr"/>

            <ul class="nav nav-pills ucenter-tab">
    <li
    <?php if(($type) == "index"): ?>class="active"<?php endif; ?>
    ><a href="<?php echo U('Ucenter/Invite/index');?>"><?php echo L('_INV_CODE_GEN_');?></a></li>
    <li
    <?php if(($type) == "invite"): ?>class="active"<?php endif; ?>
    ><a href="<?php echo U('Ucenter/Invite/invite');?>"><?php echo L('_INV_LIST_');?></a></li>
</ul>
        </div>
    </div>
    ...
    <div class="invite_content" style="min-height: 400px;margin-top: 30px;">
        <?php if(!$type_list): ?><p class="text-muted" style="text-align: center; font-size: 18px;">
            <?php echo L('_INV_NOT_');?> <a href="<?php echo U('Ucenter/Invite/index');?>" style="text-decoration: underline;"><?php echo L('_INV_CODE_GEN_');?></a> ～
        </p>
            <?php else: ?>
            <?php if(is_array($type_list)): $i = 0; $__LIST__ = $type_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$val): $mod = ($i % 2 );++$i;?><div style="font-size: 18px;"><?php echo ($val["title"]); ?></div>
                <div style="padding-left: 20px;margin-bottom: 30px;">
                    <table>
                        <thead>
                        <tr>
                            <td><?php echo L('_INV_CODE_');?></td>
                            <td><?php echo L('_INV_LINK_');?></td>
                            <td><?php echo L('_PLACES_REMIND_');?></td>
                            <td><?php echo L('_SUCCESS_ED_');?></td>
                            <td><?php echo L('_PERIOD_TO_');?></td>
                            <td><?php echo L('_OPERATE_');?></td>
                        </tr>
                        </thead>
                        <tbody>
                        <?php if(is_array($val['codes'])): $i = 0; $__LIST__ = $val['codes'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vl): $mod = ($i % 2 );++$i;?><tr>
                                <td><?php echo ($vl["code"]); ?></td>
                                <td class="text-more" style="width: 100%"><?php echo ($vl["code_url"]); ?></td>
                                <td><?php echo ((isset($vl["num"]) && ($vl["num"] !== ""))?($vl["num"]):0); echo L('_GE_');?></td>
                                <td><?php echo ((isset($vl["already_num"]) && ($vl["already_num"] !== ""))?($vl["already_num"]):0); ?> <?php echo L('_GE_');?></td>
                                <td><?php echo (time_format($vl["end_time"])); ?></td>
                                <td>
                                    <div style="position: relative;">
                                        <a data-role="copy_code" data-code="<?php echo ($vl["code"]); ?>"><?php echo L('_INV_CODE_COPY_');?></a>
                                        <a data-role="copy_code_url" data-code-url="<?php echo ($vl["code_url"]); ?>" style="margin-left: 10px;"><?php echo L('_INV_LINK_COPY_');?></a>
                                        <a data-role="back_copy_code" data-id="<?php echo ($vl["id"]); ?>" style="margin-left: 10px;"><?php echo L('_RETURN_TO_');?></a>
                                    </div>
                                </td>
    ...
    <script type="text/javascript" src="/opensnseclipse/Application/Ucenter/Static/js/invite.js"></script>
    <script type="text/javascript" src="/opensnseclipse/Public/js/ext/zclip/jquery.zclip.min.js"></script>
    <script>
        $(function(){
            $('[data-role="copy_code"]').zclip({
                ...
                afterCopy: function () {
                    $(this).html("<?php echo L('_COPIED_');?>");
                    toast.success("<?php echo L('_SUCCESS_COPY_');?>");
                }
            });
            $('[data-role="copy_code_url"]').zclip({
                ...
                afterCopy: function () {
                    $(this).html("<?php echo L('_LINK_COPIED_');?>");
                    toast.success("<?php echo L('_SUCCESS_LINK_COPY_');?>");
                }
            });
        });
    </script>
